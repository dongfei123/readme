  ...
  <body>
      ...
      <div class="layui-row">
        <form action="{{url('admin/user')}}" method="get" class="layui-form layui-col-md12 x-so">
          <div class="layui-input-inline">
            <select name="pagesize" lay-filter="aihao">
              <option value=""></option>
              <option value="3" @if($request->input('pagesize') == 3) selected  @endif>3</option>
              <option value="6" @if($request->input('pagesize') == 6) selected  @endif>6</option>
              <option value="10"  @if($request->input('pagesize') == 10) selected  @endif>10</option>
            </select>
          </div>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="td-manage">
              ...
              <a title="授予角色"  onclick="x_admin_show('授予角色','{{url('admin/user/role/'.$user->id)}}',600,400)" href="javascript:;">
                <i class="layui-icon">&#xe633;</i>
              </a>
    ...
    <script>
      ...
      function delAll (argument) {
        ...
        layer.confirm('确认要删除吗？',function(index){
            //捉到所有被选中的，发异步进行删除
            $.post('/admin/user/del',{'uids':uids,'_token':"{{csrf_token()}}"},function(data){
                if(data.status == 0){
                  layer.msg(data.message, {icon: 1,time: 1000});
